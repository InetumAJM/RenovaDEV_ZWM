...
**********
  ...
  IF sy-tcode = 'ZWM036_M'.

*    PERFORM check_quant.

    LOOP AT SCREEN.
      CHECK screen-group1 = 'EXI'.
      screen-input = '1'.
*      screen-required = '1'.
      MODIFY SCREEN.
    ENDLOOP.
...
** Obtém valores para o search-help
  ...
  CLEAR: mara.
  SELECT DISTINCT * FROM mara AS a
                    INNER JOIN makt AS b ON a~matnr EQ b~matnr
                    INTO CORRESPONDING FIELDS OF TABLE itab_f4
                    WHERE mtart EQ 'PALT'.

  SORT itab_f4 BY matnr.
  DELETE ADJACENT DUPLICATES FROM itab_f4.
...
*    DO num_pal TIMES.
    WHILE num_pal >= 1.
      ...
      IF z02rpsessao-data <> datum.
        z02rpsessao-data = datum.
      ENDIF.
      IF lgnum = '100'.
        num_pal = num_pal - 1.
        IF lhmg1 < l_lhmg1.
        ...
        ELSE.
          ...
          IF z_wm_cl_management=>is_remontada( is_data = mlgn ) eq abap_true.
            IF lhmg1 > l_lhmg1 AND num_pal IS NOT INITIAL.
              l_quantity = l_quantity + l_lhmg1.
              num_pal = num_pal - 1.
            ENDIF.
          ENDIF.
        ENDIF.
      ...
      CALL FUNCTION 'ZWM_ENTRADA_PRODUCAO_F03'
        EXPORTING
          matnr     = l_matnr
          lgnum     = lgnum
          aufnr     = l_aufnr
...
*          OTHERS     = 2.
      ...
      IF sy-subrc <> 0 OR l_msg NS 'SSCC'..
...
*                WITH sy-msgv1 sy-msgv2 sy-msgv3 sy-msgv4.
      ...
      ELSE.
        ...
        MESSAGE i000(zmsg) WITH 'Entrada de produção efectuada com sucesso.'
                                'Doc. Material criado:' l_sscc1.

        " Actualizar zwm030

        UPDATE zwm030 SET prodp = prodp + l_quantity
                      WHERE aufnr = l_aufnr.


        IF l_retorno IS INITIAL.
          IF imprimir = 'X' AND lgnum = '100'.
...
*             maktx_pal, matnr_pal,
             ...
             cursorfield, area1, linha, desc1, l_cons,
             l_valor, l_aux, l_quant, ean11, quant, datum.
...
*        l_lhmg1 LIKE mlgn-lhmg1,
  ...
  ELSEIF lgnum = '100'.
    ...
    IF l_lhmg1 IS INITIAL.
      MESSAGE e000 WITH 'Dados mestre de material não estão atualizados'.
      cursorfield = 'LHMG1'.
    ENDIF.
    IF lhmg1 > l_lhmg1.
      resto = lhmg1 MOD l_lhmg1.
    ENDIF.

    IF resto IS NOT INITIAL.
      CLEAR lhmg1.
      MESSAGE e117.
      cursorfield = 'LHMG1'.
    ELSE.
      num_pal = lhmg1 DIV l_lhmg1.
      IF num_pal IS INITIAL.
        num_pal = 1.
      ENDIF.
    ENDIF.
  ENDIF.
...
FORM check_area .
  ...
  ELSE.
    cursorfield = 'AREA'.
    MESSAGE e000 WITH 'A Área' area1 'não existe'.
  ENDIF.
