...
*                                                                      *
************************************************************************
* Nome ABAP.: ZWMREP0024 2.0                                           *
* Nm.Cliente: RENOVA                                                   *
* Descrição.: Cockpit de Expedição                                     *
* Criado por: Sérgio Garcias                                           *
* Criado em.: 2023/09/19                                               *
* Tipo PRG..: Report ALV Tree                                          *
************************************************************************

REPORT zwmrep0024b NO STANDARD PAGE HEADING MESSAGE-ID zwmmsg001.

*--------------------------------------------------- Dados Globais
INCLUDE zwmrep0024b_d01.

*----------------------------------------------------Ecran de selecção
INCLUDE zwmrep0024b_i01.
...
FORM check_grupo USING f_grupo f_subrc.
  ...
  IF z_wm_cl_management=>is_group_completed( is_data = t311 ) <> abap_true.
    IF ls_zwm028-x_epe <> 1.
      MESSAGE ID 'ZWM001' TYPE 'I' NUMBER '097' WITH f_grupo.
      f_subrc = 9.
      EXIT.
    ENDIF.
  ENDIF.
    ...
    IF lgnum EQ '100'.
      CALL METHOD mr_toolbar->add_button
        EXPORTING
          ...
          butn_type = cntb_btype_button
          text      = 'Lib. AUT'
          quickinfo = 'Libertar Trabalho Automático'.
      ...
      CALL METHOD mr_toolbar->add_button
        EXPORTING
          ...
          butn_type = cntb_btype_button
          text      = 'Idoc AUT'
          quickinfo = 'Reenviar Idoc Automático'.
    ENDIF.
  ENDIF.
...
* Expand first level
  LOOP AT gt_expand_node INTO lv_node_key.
...
*  aux_cat-do_sum         = 'X'.
  ...
  CLEAR aux_cat.
  aux_cat-col_pos       =  pos.
  aux_cat-fieldname     = 'ICON_CREATED'.
*  aux_cat-no_out        = 'X'.
  aux_cat-icon           = 'X'.
  aux_cat-outputlen     =  15.
  aux_cat-coltext       = 'Status Criação'(078).
  aux_cat-tooltip       = 'Status da Criação'(079).
  APPEND aux_cat.
  ADD 1 TO pos.
...
*  aux_cat-key           = 'X'.
  ...
  aux_cat-outputlen     =  2.
  aux_cat-coltext       = 'Pulmao Pal.'(042).
  APPEND aux_cat.
  ADD 1 TO pos.
  ...
  aux_cat-outputlen     =  6.
  aux_cat-coltext       = 'CRD Pal.'(047).
  APPEND aux_cat.
  ADD 1 TO pos.

  CLEAR aux_cat.
  aux_cat-col_pos       =  pos.
  aux_cat-fieldname     = 'PALETES_PCK'.
  aux_cat-outputlen     =  6.
  aux_cat-coltext       = 'PCK Pal.'(048).
  APPEND aux_cat.
  ADD 1 TO pos.

  CLEAR aux_cat.
  aux_cat-col_pos       =  pos.
  aux_cat-fieldname     = 'PALETES_BLK'.
  aux_cat-outputlen     =  6.
  aux_cat-coltext       = 'BLK Pal.'(076).
  APPEND aux_cat.
  ADD 1 TO pos.

  CLEAR aux_cat.
  aux_cat-col_pos       =  pos.
  aux_cat-fieldname     = 'PALETES_BPK'.
  aux_cat-outputlen     =  6.
  aux_cat-coltext       = 'BPK Pal.'(080).
  APPEND aux_cat.
  ADD 1 TO pos.
  ...
  CLEAR aux_cat.
  aux_cat-col_pos       =  pos.
  aux_cat-fieldname     = 'IDOCNUM'.
  aux_cat-no_zero       = abap_true.
  aux_cat-outputlen     =  10.
  aux_cat-coltext       = 'Nº do IDoc'.
  APPEND aux_cat.
  ADD 1 TO pos.

  CLEAR aux_cat.
  aux_cat-col_pos       =  pos.
  aux_cat-fieldname     = 'IDOCNUM_LIB'.
  aux_cat-no_zero       = abap_true.
  aux_cat-outputlen     =  10.
  aux_cat-coltext       = 'IDoc de Liberação'.
  APPEND aux_cat.
  ADD 1 TO pos.
...
FORM create_hierarchy.
  ...
  FIELD-SYMBOLS: <ls_out> TYPE t_out.

  CLEAR: gt_expand_node.
...
** Não lista se não existirem dados
  ...
  LOOP AT tab_out ASSIGNING <ls_out> WHERE idocnum_lib IS INITIAL.
    LOOP AT tab_out INTO ls_out FROM sy-tabix
                                WHERE vbeln = <ls_out>-vbeln AND
                                      NOT idocnum_lib IS INITIAL.
      <ls_out>-idocnum_lib = ls_out-idocnum_lib.
      EXIT.
    ENDLOOP.
  ENDLOOP.
  ...
  LOOP AT tab_out INTO ls_out.
    ...
    AT NEW all.
      ...
      APPEND l_all_key TO gt_expand_node.
    ENDAT.
    lsx_out = ls_out.
    ...
    AT NEW vkorg.
      ...
      APPEND l_vkorg_key TO gt_expand_node.
    ENDAT.
    ...
    IF NOT ls_out-tanum IS INITIAL.
...
** Paletes DRI, TRI, PRM e Picking
  CLEAR: paletes_dri, paletes_tri, paletes_prm, paletes_crd, paletes_aut,
         paletes_blk, paletes_pck, paletes_bpk.
  ...
  LOOP AT lt_paletes.
    ...
    paletes_blk = paletes_blk + lt_paletes-pal_blk.
    paletes_pck = paletes_pck + lt_paletes-pal_pck.
    paletes_bpk = paletes_bpk + lt_paletes-pal_bpk.
  ENDLOOP.
  ...
  WRITE paletes_blk TO ls_out-paletes_blk DECIMALS 0.
  WRITE paletes_pck TO ls_out-paletes_pck DECIMALS 0.
  WRITE paletes_bpk TO ls_out-paletes_bpk DECIMALS 0.
...
*  SORT lt_transportes BY refnr.
  ...
  CLEAR zwm028.
  READ TABLE gt_zwm028 WITH KEY refnr   = ps_out-refnr
                                remessa = ''.
...
*         AND    remessa  = space.
  ...
  IF sy-subrc = 0.
    ...
    PERFORM atribui_icon_lock_grupo USING zwm028-refnr zwm028-zlock
                                    CHANGING ls_out-icon_lock ls_out-icon_created.
  ...
  ELSE.
    LOOP AT gt_zwm028 WHERE refnr = ps_out-refnr.

*    SELECT total_paletes INTO zwm028-total_paletes FROM  zwm028
*           WHERE  lgnum    = lgnum
*           AND    refnr    = ps_out-refnr.

      ADD gt_zwm028-total_paletes  TO ls_out-total_paletes.
      ADD gt_zwm028-paletes_pulmao TO ls_out-paletes_pulmao.
    ENDLOOP.
*  ENDSELECT.

    PERFORM atribui_icon_lock_grupo USING ls_out-refnr 0
                                    CHANGING ls_out-icon_lock ls_out-icon_created.
  ENDIF.
...
** Prioridade
  ...
  MOVE zwm028-free_idoc  TO ls_out-idocnum_lib.
...
* set item-layout
  ...
  MOVE ps_out-idocnum_lib TO ls_out-idocnum_lib.

  CLEAR: ls_out-total_paletes, ls_out-paletes_pulmao.

  CLEAR: zwm028-total_paletes, zwm028-paletes_pulmao.

  READ TABLE gt_zwm040 WITH KEY refnr   = ps_out-refnr
                                remessa = ps_out-vbeln.
  IF sy-subrc = 0.
    lv_vbeln = gt_zwm040-id_servisan.
  ELSE.
    lv_vbeln = ps_out-vbeln.
  ENDIF.

*  SELECT SINGLE id_servisan
*    FROM zwm040 INTO lv_vbeln
*   WHERE lgnum   EQ lgnum
*     AND refnr   EQ ps_out-refnr
*     AND remessa EQ ps_out-vbeln.
*
...
*         WHERE  lgnum    = lgnum
*         AND    refnr    = ps_out-refnr
*         AND    remessa  = lv_vbeln.
  IF sy-subrc = 0.
    MOVE gt_zwm028-total_paletes  TO ls_out-total_paletes.
    MOVE gt_zwm028-paletes_pulmao TO ls_out-paletes_pulmao.
    MOVE gt_zwm028-free_idoc      TO ls_out-idocnum_lib.
  ENDIF.
...
FORM build_comment USING
  ...
  CLEAR ls_line.
  ls_line-typ  = 'H'.
  ls_line-info = 'Cockpit de Expedição'(t00).
...
FORM select_data.
  ...
  DATA: lt_all_tasks LIKE all_tasks[],
        ls_all_tasks LIKE all_tasks.
  ...
  DATA: lt_vbss   TYPE TABLE OF vbss   WITH HEADER LINE.
...
**********************************************************************
  ...
  REFRESH: itab_28, itab_serv, lt_t311, gt_zwm028, gt_zwm040, lt_t311a, r_id.
  CLEAR:   itab_28, itab_serv, lt_t311, gt_zwm028, gt_zwm040, lt_t311a, r_id.
  ...
  IF NOT lt_t311[] IS INITIAL.
    ...
    SORT gt_zwm028 BY refnr remessa.

    SELECT * INTO TABLE gt_zwm040
    FROM zwm040
        FOR ALL ENTRIES IN lt_t311
            WHERE lgnum = lt_t311-lgnum AND
                  refnr = lt_t311-refnr.

    SORT gt_zwm040 BY refnr remessa.
...
** Get carros já registados na portaria
  ...
  LOOP AT lt_t311.
    ...
    READ TABLE lt_vbss WITH KEY sammg = lt_t311-refnr.
    IF sy-subrc = 0.
      READ TABLE lt_vttp WITH KEY vbeln = lt_vbss-vbeln.
      IF sy-subrc = 0.
        transporte-low = lt_vttp-tknum.
      ENDIF.
    ENDIF.

    CLEAR lt_vttk.
    READ TABLE lt_vttk WITH KEY tknum = transporte-low.

** Validar tipo de grupo
**********************************************************************
    LOOP AT lt_t311a WHERE refnr = lt_t311-refnr.

      READ TABLE gt_group_rem WITH KEY refnr = lt_t311a-refnr
                                       vbeln = lt_t311a-rbnum.
      ...
      IF sy-subrc <> 0.
        ...
        CALL FUNCTION 'ZWM_GROUP_CHECK_2STEP'
          EXPORTING
            i_lgnum  = lt_t311a-lgnum
            i_refnr  = lt_t311a-refnr
        ...
        CLEAR gt_group_rem.
        gt_group_rem-refnr  = lt_t311a-refnr.
        gt_group_rem-vbeln  = lt_t311a-rbnum.
        gt_group_rem-2step  = lv_2step.
        gt_group_rem-2spart = lv_2spart.
        APPEND gt_group_rem.

      ELSE.
        lv_2step  = gt_group_rem-2step.
        lv_2spart = gt_group_rem-2spart.
      ENDIF.
...
*        OTHERS           = 3.
  ...
  SORT gr_refnr_2step  BY low.
  SORT gr_refnr_2spart BY low.
  SORT gr_refnr        BY low.

  DELETE ADJACENT DUPLICATES FROM gr_refnr_2step  COMPARING low.
  DELETE ADJACENT DUPLICATES FROM gr_refnr_2spart COMPARING low.
  DELETE ADJACENT DUPLICATES FROM gr_refnr        COMPARING low.
...
**********************************************************************
  ...
  IF NOT gr_refnr_2spart IS INITIAL.
    SELECT a~lgnum a~tanum a~benum a~refnr a~kquit a~queue a~kgvnq
           ...
           e~num_recorrido f~vkorg e~sscc a~benum c~nltyp d~free_idoc
    APPENDING CORRESPONDING FIELDS OF TABLE lt_all_tasks
    ...
    WHERE a~lgnum = lgnum  AND
          ...
          ( a~queue = picking_queue OR
            ...
            a~queue = queue_crd     OR
            a~queue = aut_queue     OR
            a~queue = aut_queue_tri ).

    LOOP AT lt_all_tasks INTO ls_all_tasks WHERE refnr IN gr_refnr_2spart.
      IF ls_all_tasks-vltyp <> '815' AND ls_all_tasks-nltyp EQ '916'.
        DELETE lt_all_tasks INDEX sy-tabix.
        CONTINUE.
      ENDIF.

      ls_all_tasks-vbeln = ls_all_tasks-benum.
      MODIFY lt_all_tasks FROM ls_all_tasks INDEX sy-tabix TRANSPORTING vbeln.
    ENDLOOP.

    APPEND LINES OF lt_all_tasks TO all_tasks.
...
** Grupos Sem Tarefas
***********************************************************************
  REFRESH: lt_all_tasks.

  SELECT a~lgnum a~refnr f~vbeln b~refnt f~vkorg
  APPENDING CORRESPONDING FIELDS OF TABLE lt_all_tasks
  FROM t311a AS a
       INNER JOIN t311 AS b
             ON a~lgnum = b~lgnum AND
                a~refnr = b~refnr
       INNER JOIN likp AS f
             ON f~vbeln = a~rbnum
  WHERE a~lgnum = lgnum  AND
        a~refnr IN refnr AND
        b~refnt IN refnt AND
        b~datum IN datum AND
        f~vkorg IN vkorg.
  ...
  IF lt_all_tasks[] IS NOT INITIAL.
    SELECT vbeln lvstk
     ...
     FOR ALL ENTRIES IN lt_all_tasks
      WHERE vbeln =  lt_all_tasks-vbeln.

    DELETE lt_vbuk WHERE lvstk IS NOT INITIAL AND lvstk <> 'A'.
  ENDIF.

  LOOP AT lt_all_tasks INTO ls_all_tasks.
    lv_tabix = sy-tabix.

    " Remessa com tarefas criadas
    READ TABLE lt_vbuk WITH KEY vbeln = ls_all_tasks-vbeln.
    IF sy-subrc <> 0.
      DELETE lt_all_tasks INDEX lv_tabix.
      CONTINUE.
    ENDIF.

    " Grupo com tarefas
    IF ls_all_tasks-refnr IN gr_refnr_2step AND gr_refnr_2step[] IS NOT INITIAL.
      READ TABLE all_tasks TRANSPORTING NO FIELDS WITH KEY refnr = ls_all_tasks-refnr.
      IF sy-subrc = 0.
        DELETE lt_all_tasks INDEX lv_tabix.
        CONTINUE.
      ENDIF.
  ...
  SELECT a~lgnum a~tanum a~vbeln a~refnr a~kquit a~queue a~kgvnq
        ...
        c~tapos c~vltyp c~vlpla c~matnr c~vsolm c~nsolm c~meins c~pquit
         ...
         d~porta d~zlock d~remessa
         d~prioridade d~paletes_carro
         e~num_recorrido f~vkorg e~sscc d~free_idoc
...
***********************************************************************
  ...
  SELECT a~lgnum a~tanum a~benum a~refnr a~kquit a~queue a~kgvnq
         ...
         c~posty c~vlenr c~maktx c~posnr c~letyp c~vista
         d~st_pul d~pulmao1 d~pulmao2 d~st_ppk d~pre_pick d~st_dck
         d~porta d~zlock e~remessa
         d~prioridade d~paletes_carro
         e~num_recorrido f~vkorg e~sscc a~benum c~nltyp d~free_idoc
  INTO CORRESPONDING FIELDS OF TABLE lt_all_tasks
  FROM ( ( ( ( ( t311 AS b INNER JOIN ltak AS a
         ON b~lgnum = a~lgnum AND
            b~refnr = a~refnr )
             INNER JOIN ltap AS c
  ...
  SORT lt_all_tasks BY tanum.
  DELETE ADJACENT DUPLICATES FROM lt_all_tasks COMPARING vkorg tanum.

  DELETE lt_all_tasks WHERE vlenr IS INITIAL.
  IF NOT lt_all_tasks[] IS INITIAL.

    SELECT * FROM zwm026
             INTO TABLE lt_zwm026
             FOR ALL ENTRIES IN lt_all_tasks
             WHERE armazem = lgnum AND
                   sscc = lt_all_tasks-vlenr.
    SORT lt_zwm026 BY sscc.

    LOOP AT lt_all_tasks INTO ls_all_tasks.
      READ TABLE all_tasks
        WITH KEY tanum = ls_all_tasks-tanum
        BINARY SEARCH.
      CHECK sy-subrc <> 0.

      IF NOT ls_all_tasks-vlenr IS INITIAL.
        CLEAR: lt_zwm026.
        READ TABLE lt_zwm026
              WITH KEY sscc = ls_all_tasks-vlenr.
        IF sy-subrc EQ 0.
          ls_all_tasks-vbeln = lt_zwm026-remessa.
        ENDIF.
      ENDIF.

      IF ls_all_tasks-vbeln IS INITIAL.
        ls_all_tasks-vbeln = z_wm_cl_management=>get_vbeln( is_data = ls_all_tasks ).
      ENDIF.
      CHECK NOT ls_all_tasks-vbeln IS INITIAL.

      READ TABLE all_tasks
        WITH KEY vbeln = ls_all_tasks-vbeln.
      IF sy-subrc EQ 0.
        ls_all_tasks-vkorg = all_tasks-vkorg.
      ENDIF.

      APPEND ls_all_tasks TO all_tasks.
      SORT all_tasks BY tanum.
    ENDLOOP.
  ENDIF.
...
**********************************************************************
  LOOP AT all_tasks.
    ...
    IF all_tasks-refnr IN gr_refnr_2step AND gr_refnr_2step[] IS NOT INITIAL.

      READ TABLE gt_zwm040 WITH KEY refnr = all_tasks-refnr.
...
*      AND remessa EQ zwm040-id_servisan.
    ...
    READ TABLE gt_zwm028 WITH KEY refnr   = all_tasks-refnr
                                  remessa = gt_zwm040-id_servisan.

    IF sy-subrc EQ 0.
      all_tasks-st_pul      = gt_zwm028-st_pul.
      all_tasks-pulmao1     = gt_zwm028-pulmao1.
      all_tasks-pulmao2     = gt_zwm028-pulmao2.
      all_tasks-kober       = gt_zwm028-kober.
      all_tasks-st_ppk      = gt_zwm028-st_ppk.
      all_tasks-pre_pick    = gt_zwm028-pre_pick.
      all_tasks-st_dck      = gt_zwm028-st_dck.
      all_tasks-porta       = gt_zwm028-porta.
      all_tasks-zlock       = gt_zwm028-zlock.
      all_tasks-prioridade  = gt_zwm028-prioridade.
*      all_tasks-free_idoc   = gt_zwm028-free_idoc.
    ENDIF.
...
** Excluir consoante as opções de selecção
  IF refnt[] IS NOT INITIAL.
    DELETE all_tasks WHERE refnt NOT IN refnt.
  ENDIF.
...
*---------------------------------------------------------------------*
FORM tab_final.
  TYPES: BEGIN OF lty_remontada_index,
           index TYPE sytabix,
           lenum TYPE lenum,
         END OF lty_remontada_index.

  TYPES: BEGIN OF lty_remontada_idoc,
           lenum TYPE lenum,
           idoc  TYPE edi_docnum,
         END OF lty_remontada_idoc.
  ...
  DATA: lt_t311a_calc      TYPE TABLE OF t311a,
        ...
        lt_t311a_serv      TYPE TABLE OF t311a,
        lt_t311a_pesp      TYPE TABLE OF t311a,
        lt_remontada_idoc  TYPE TABLE OF lty_remontada_idoc,
        lt_remontada_index TYPE TABLE OF lty_remontada_index,
        lt_all_tasks       LIKE TABLE OF all_tasks WITH HEADER LINE,
        lt_ltap_epe        TYPE TABLE OF lty_ltap  WITH HEADER LINE,
        lt_ltap_aut        TYPE TABLE OF lty_ltap  WITH HEADER LINE,
        lt_marm            TYPE TABLE OF marm      WITH HEADER LINE,
        lt_mara            TYPE TABLE OF lty_mara  WITH HEADER LINE,
*        lt_vepo            TYPE TABLE OF vepo      WITH HEADER LINE,
        lt_pal_kna1        TYPE TABLE OF lty_kna1  WITH HEADER LINE,
        lt_tvkot           TYPE TABLE OF tvkot     WITH HEADER LINE,
        lt_adrc            TYPE TABLE OF adrc      WITH HEADER LINE,
        lt_zwm013          TYPE TABLE OF zwm013    WITH HEADER LINE.
  ...
  CLEAR: lt_paletes, paletes_dri, paletes_tri, paletes_prm, paletes_crd, paletes_aut, paletes_blk,
         paletes_pck.
...
** Textos
  SELECT *
    FROM tvkot INTO TABLE lt_tvkot
...
** Clientes
  IF lt_t311a[] IS NOT INITIAL.
    SELECT vbeln kunnr
      FROM likp INTO TABLE lt_pal_kna1
      FOR ALL ENTRIES IN lt_t311a
      WHERE vbeln = lt_t311a-rbnum.
  ENDIF.
...
** Paletes de Picking
  IF lt_t311[] IS NOT INITIAL.
    SELECT *
      ...
      FOR ALL ENTRIES IN lt_t311
      WHERE armazem = lgnum
      AND   grupo   = lt_t311-refnr
      AND   estado  <> 'T'.

    DELETE lt_zwm026 WHERE pal_picking IS INITIAL.

    SORT lt_zwm026 BY grupo remessa n_pal_picking.
    DELETE ADJACENT DUPLICATES FROM lt_zwm026 COMPARING grupo remessa n_pal_picking.
  ENDIF.

** Materiais - Unidade PAL
  lt_all_tasks[] = all_tasks[].

  DELETE lt_all_tasks WHERE matnr IS INITIAL.

  SORT lt_all_tasks BY matnr.
  DELETE ADJACENT DUPLICATES FROM lt_all_tasks COMPARING matnr.

  IF lt_all_tasks[] IS NOT INITIAL.
    SELECT *
      FROM marm INTO TABLE lt_marm
      FOR ALL ENTRIES IN lt_all_tasks
      WHERE matnr = lt_all_tasks-matnr.
*      AND   meinh = 'PAL'.
  ...
  LOOP AT lt_t311.
    ...
    LOOP AT lt_t311a INTO ls_t311a WHERE refnr = lt_t311-refnr.

      " Paletes Completas
      LOOP AT all_tasks WHERE refnr = ls_t311a-refnr
                          AND vbeln = ls_t311a-rbnum
                          AND pquit = ' '
                          AND pvqui = ' '.
...
*          meinh = 'PAL'.
          READ TABLE lt_marm WITH KEY matnr = all_tasks-matnr
                                      meinh = 'PAL' BINARY SEARCH.
          IF sy-subrc = 0.

            qtd_pal = lt_marm-umrez / lt_marm-umren.
...
*            IF z_wm_cl_management=>is_picking( i_lgnum = lgnum is_data = all_tasks ).
*              qtd_pal = 1.
*            ENDIF.
            ...
            IF all_tasks-vltyp = 'DRI' OR
              ...
              IF all_tasks-letyp IN z_wm_cl_management=>r_letyp_remontada( all_tasks-lgnum ).
*              IF z_wm_cl_management=>is_remontada( is_data = all_tasks ) EQ abap_true.
                qtd_pal = qtd_pal / 2.
              ENDIF.
            ENDIF.
            ...
            CASE all_tasks-vltyp.
              ...
              WHEN 'BLK'.
                paletes_blk = paletes_blk + qtd_pal.
              WHEN 'BPK'.
                paletes_bpk = paletes_bpk + qtd_pal.
            ENDCASE.
          ENDIF.
        ENDIF.
      ENDLOOP.
      ...
      " Paletes picking
      LOOP AT lt_zwm026 INTO ls_zwm026 WHERE grupo   = ls_t311a-refnr
                                       AND   remessa = ls_t311a-rbnum.

        paletes_pck = paletes_pck + ls_zwm026-pal_picking.
      ENDLOOP.

      " Obter contagem de Paletes
      lt_paletes-refnr = ls_t311a-refnr.
      lt_paletes-vbeln = ls_t311a-rbnum.

      READ TABLE lt_pal_kna1 WITH KEY vbeln = lt_paletes-vbeln.
      IF sy-subrc = 0.
        lt_paletes-kunnr = lt_pal_kna1-kunnr.
      ENDIF.
      ...
      lt_paletes-pal_blk = paletes_blk.
      lt_paletes-pal_pck = paletes_pck.
      lt_paletes-pal_bpk = paletes_bpk.
      COLLECT lt_paletes.

      CLEAR: lt_paletes, paletes_dri,
      paletes_tri, paletes_prm,
      paletes_crd, paletes_aut,
      paletes_blk, paletes_pck,
      paletes_bpk.
    ENDLOOP.
  ENDLOOP.
...
** Validar Paletes Pal. Especial
  ...
  IF lt_all_tasks[] IS NOT INITIAL.
    SELECT lgnum tanum tapos vltyp nlenr kzsub
      FROM ltap INTO TABLE lt_ltap_epe
      FOR ALL ENTRIES IN lt_all_tasks
      WHERE lgnum = lt_all_tasks-lgnum
      AND   nlenr = lt_all_tasks-vlenr
      AND   pquit = 'X'.

    DELETE lt_ltap_epe WHERE vltyp <> 'EPE'.

    SORT lt_ltap_epe BY nlenr.

    SELECT *
      FROM zwm013 INTO TABLE lt_zwm013
      FOR ALL ENTRIES IN lt_all_tasks
      WHERE armazem = lt_all_tasks-lgnum
      AND   sscc    = lt_all_tasks-vlenr.
  ENDIF.

  lt_all_tasks[] = all_tasks[].
  DELETE lt_all_tasks WHERE sscc IS INITIAL.
  ...
  IF lt_all_tasks[] IS NOT INITIAL.
    SELECT *
      ...
      FOR ALL ENTRIES IN lt_all_tasks
      WHERE armazem = lt_all_tasks-lgnum
      AND   sscc    = lt_all_tasks-sscc.
  ENDIF.

  SORT lt_zwm013 BY sscc.

** Valida IDOC OT do automático
  lt_all_tasks[] = all_tasks[].
  DELETE lt_all_tasks WHERE tanum IS INITIAL.

  IF lt_all_tasks[] IS NOT INITIAL.
    SELECT lgnum tanum tapos vltyp nlenr kzsub
      FROM ltap INTO TABLE lt_ltap_aut
      FOR ALL ENTRIES IN lt_all_tasks
      WHERE lgnum = lt_all_tasks-lgnum
      AND   tanum = lt_all_tasks-tanum.

    SORT lt_ltap_aut BY tanum.
  ENDIF.

** Validar tipo do grupo
*  lt_all_tasks[] = all_tasks[].
*
*  SORT lt_all_tasks BY refnr vbeln.
*  DELETE ADJACENT DUPLICATES FROM lt_all_tasks COMPARING refnr vbeln.
*
*  LOOP AT lt_all_tasks.
*    lv_tabix = sy-tabix.
*
*    CALL FUNCTION 'ZWM_GROUP_CHECK_2STEP'
*      EXPORTING
*        i_lgnum  = lt_all_tasks-lgnum
*        i_refnr  = lt_all_tasks-refnr
*        i_vbeln  = lt_all_tasks-vbeln
*      IMPORTING
*        e_2step  = lv_2step
*        e_2spart = lv_2spart
*      EXCEPTIONS
...
*        OTHERS   = 2.
*
*    lt_all_tasks-pick_2step  = lv_2step.
*    lt_all_tasks-pick_2spart = lv_2spart.
*
*    MODIFY lt_all_tasks INDEX lv_tabix TRANSPORTING pick_2step pick_2spart.
*  ENDLOOP.

** Clientes Paletização Especial
  IF NOT lt_t311a[] IS INITIAL.
    SELECT * FROM vbpa
       INTO TABLE lt_vbpa
       FOR ALL ENTRIES IN lt_t311a
       WHERE vbeln = lt_t311a-rbnum AND
             posnr = '000000' AND
...
** Clientes paletização especial
  LOOP AT lt_zwm049 INTO ls_zwm049 WHERE paltxt IS NOT INITIAL.

    LOOP AT lt_vbpa INTO ls_vbpa WHERE kunnr = ls_zwm049-kunnr.

      READ TABLE lt_t311a INTO ls_t311a WITH KEY rbnum = ls_vbpa-vbeln.
      CHECK sy-subrc = 0.

      CLEAR lt_aux.
      lt_aux-refnr           = ls_t311a-refnr.
      lt_aux-vbeln           = ls_t311a-rbnum.
      lt_aux-palete_especial = 'X'.
      COLLECT lt_aux.
    ENDLOOP.
  ENDLOOP.

  LOOP AT lt_zwm031 INTO ls_zwm031.

    LOOP AT lt_vbpa INTO ls_vbpa WHERE kunnr = ls_zwm031-kunnr.
  ...
  SORT lt_aux BY refnr vbeln.

** Clientes Servisan
  LOOP AT lt_zwm039 INTO ls_zwm039.

    LOOP AT lt_vbpa INTO ls_vbpa WHERE kunnr = ls_zwm039-kunnr.

      READ TABLE lt_t311a INTO ls_t311a WITH KEY rbnum = ls_vbpa-vbeln.
      CHECK sy-subrc = 0.

      APPEND ls_t311a TO lt_t311a_serv.
...
**********************************************************************
  LOOP AT all_tasks.
    ...
    IF NOT all_tasks-tanum IS INITIAL.
    ...
    WRITE:
           ...
           all_tasks-free_idoc  TO tab_out-idocnum_lib.

    IF all_tasks-refnt CS gc_incomplete_tag.
      WRITE 'Incompletos' TO tab_out-all.
    ENDIF.
...
*                    USING all_tasks-refnr all_tasks-zlock
*                 CHANGING tab_out-icon_lock tab_out-icon_created.
*
...
*                 CHANGING tab_out-desc_lock.
    ...
    READ TABLE lt_tvkot WITH KEY vkorg = all_tasks-vkorg.
*
*    SELECT SINGLE * FROM  tvkot
*           WHERE  spras  = sy-langu
*           AND    vkorg  = all_tasks-vkorg.

    CONCATENATE  all_tasks-vkorg '-' lt_tvkot-vtext INTO tab_out-vkorg_out.
...
*                    meinh = 'PAL'.
      ...
      IF sy-subrc = 0.
        DO 1 TIMES.
          ...
          IF all_tasks-vltyp = 'AUT'.

            " Não Valida OT partidas
            READ TABLE lt_ltap_epe WITH KEY nlenr = all_tasks-vlenr.
            IF sy-subrc = 0.
              EXIT.
            ENDIF.
          ENDIF.
          ...
          IF qtd_pal NE 0.
            ...
            IF z_wm_cl_management=>is_picking( i_lgnum = lgnum is_data = lt_to_part ) EQ abap_false.
              APPEND lt_to_part.
            ENDIF.
...
** Paletes picking
    IF all_tasks-vltyp = 'PCK' OR all_tasks-vltyp = 'PKB'.
      ...
      READ TABLE lt_mara WITH KEY matnr = all_tasks-matnr BINARY SEARCH.
*
...
*                          WHERE matnr =  all_tasks-matnr.
      ...
      READ TABLE lt_marm WITH KEY matnr = all_tasks-matnr
                                  meinh = lt_mara-meins BINARY SEARCH.
*
*      SELECT SINGLE *
*          FROM marm
*              WHERE matnr = all_tasks-matnr AND
...
*
*    READ TABLE lt_all_tasks WITH KEY refnr = all_tasks-refnr
*                                     vbeln = all_tasks-vbeln
*                                     BINARY SEARCH.
*    IF sy-subrc = 0.
*      lv_2step  = lt_all_tasks-pick_2step.
*      lv_2spart = lt_all_tasks-pick_2spart.
*    ENDIF.
*
**    CALL FUNCTION 'ZWM_GROUP_CHECK_2STEP'
**      EXPORTING
...
**      READ TABLE lt_t311a WITH KEY refnr = all_tasks-refnr.
**
**      READ TABLE lt_vbpa WITH KEY vbeln =  lt_t311a-rbnum.
*
...
*    ELSE.
*
**      READ TABLE lt_vbpa INTO ls_vbpa WITH KEY vbeln = all_tasks-vbeln.
**
**      IF sy-subrc = 0.
**        APPEND ls_vbpa TO lt_vbpa_g.
**      ENDIF.
*
*      SELECT * FROM vbpa
*         INTO TABLE lt_vbpa
...
*
    ...
    READ TABLE lt_vbpa INTO ls_vbpa WITH KEY vbeln = all_tasks-vbeln BINARY SEARCH.
*
*    READ TABLE lt_aux WITH KEY refnr = all_tasks-refnr
*                               vbeln = all_tasks-vbeln.
*    IF sy-subrc <> 0.
*      READ TABLE lt_zwm031 WITH KEY kunnr = ls_vbpa-kunnr
*                                    matnr = all_tasks-matnr BINARY SEARCH TRANSPORTING NO FIELDS.
*      IF sy-subrc = 0.
*        tab_out-palete_especial = 'X'.
*
*        lt_aux-refnr           = all_tasks-refnr.
...
*
*      ELSE.
*        READ TABLE lt_zwm049 INTO ls_zwm049 WITH KEY kunnr = ls_vbpa-kunnr BINARY SEARCH.
*        IF sy-subrc = 0 AND ls_zwm049-paltxt IS NOT INITIAL.
*
*          tab_out-palete_especial = 'X'.
*
*          lt_aux-refnr           = all_tasks-refnr.
...
*    ENDIF.
    ...
    tab_out-kunnr = ls_vbpa-kunnr.

    READ TABLE lt_adrc WITH KEY addrnumber = ls_vbpa-adrnr.
    IF sy-subrc = 0.
      adrc-name1 = lt_adrc-name1.
    ENDIF.
...
*    ENDIF.
*
*    IF ( lt_zwm039[] IS NOT INITIAL AND all_tasks-vkorg <> 'RP08' )
*      OR all_tasks-vkorg = 'SER1'.
*      lv_servisan = 'X'.
*    ENDIF.

** Servisan
    READ TABLE lt_t311a_serv WITH KEY refnr = all_tasks-refnr
                                      rbnum = all_tasks-vbeln TRANSPORTING NO FIELDS.

    IF  ( sy-subrc = 0 AND all_tasks-vkorg <> 'RP08')
    OR all_tasks-vkorg = 'SER1'.
      lv_servisan = 'X'.
    ENDIF.

    IF all_tasks-pick_2step EQ abap_true AND lv_servisan = 'X'.
...
*        OTHERS           = 3.
    ...
    READ TABLE lt_transportes WITH KEY refnr = tab_out-refnr.

    IF sy-subrc EQ 0.
*      READ TABLE transporte INDEX 1.

*      tab_out-transporte = transporte-low.
      tab_out-transporte = lt_transportes-tknum.
    ...
    IF all_tasks-pvqui NE space.
      ...
      IF all_tasks-vlenr NE space.
        READ TABLE lt_zwm013 WITH KEY sscc = all_tasks-vlenr.
        IF sy-subrc = 0.
          tab_out-posicao_pulmao = lt_zwm013-posicao_pulmao.
        ENDIF.
...
*        ENDSELECT.
      ...
      ELSEIF all_tasks-sscc NE space.
        ...
        READ TABLE lt_zwm013 WITH KEY sscc = all_tasks-sscc.
        IF sy-subrc = 0.
          tab_out-posicao_pulmao = lt_zwm013-posicao_pulmao.
        ENDIF.
...
*        ENDSELECT.
  ...
  SORT tab_out BY all vkorg refnr vbeln tanum tapos.
  DELETE ADJACENT DUPLICATES FROM tab_out COMPARING all vkorg refnr vbeln tanum tapos.
...
*    READ TABLE lt_aux WITH KEY refnr = tab_out-refnr
*                               vbeln = tab_out-vbeln.
*    IF sy-subrc = 0.
*      tab_out-palete_especial = 'X'.
*    ENDIF.
...
** IDOC de OT do Automático
    IF NOT tab_out-tanum IS INITIAL.

      READ TABLE lt_ltap_aut WITH KEY tanum = tab_out-tanum BINARY SEARCH.
      IF sy-subrc = 0 AND lt_ltap_aut-kzsub IS NOT INITIAL.
        lv_tanum = tab_out-tanum.

*      IF z_wm_cl_management=>is_remontada( EXPORTING i_lgnum = lgnum is_data = tab_out is_p2 = abap_true IMPORTING e_lenum = lv_lenum e_sister = lv_sister ).
*        ls_remontada_index-index = l_index.
*        ls_remontada_index-lenum = lv_lenum.
*        APPEND ls_remontada_index TO lt_remontada_index.
*      ELSE.
        CALL FUNCTION 'ZWM_TO_GET_IDOC'
          EXPORTING
            ...
            i_tanum   = lv_tanum
          IMPORTING
            e_idocnum = tab_out-idocnum.

*        IF z_wm_cl_management=>is_remontada( EXPORTING i_lgnum = lgnum is_data = tab_out is_p1 = abap_true IMPORTING e_sister = lv_sister ).
*          ls_remontada_idoc-lenum = lv_sister.
*          ls_remontada_idoc-idoc  = tab_out-idocnum.
*          APPEND ls_remontada_idoc TO lt_remontada_idoc.
*        ENDIF.
*      ENDIF.
    ...
    ENDIF.
*
*    IF tab_out-idocnum_lib IS INITIAL.
*      z_wm_cl_management=>is_idoc_refnr_send( EXPORTING i_lgnum = lgnum is_data = tab_out IMPORTING e_idocnum = tab_out-idocnum_lib ).
*    ENDIF.
  ...
  SORT lt_remontada_index BY lenum.

  LOOP AT lt_remontada_idoc INTO ls_remontada_idoc.
    READ TABLE lt_remontada_index
          INTO ls_remontada_index
          WITH KEY lenum = ls_remontada_idoc-lenum
          BINARY SEARCH.
    CHECK sy-subrc EQ 0.

    READ TABLE tab_out INDEX ls_remontada_index-index.
    CHECK sy-subrc EQ 0.

    tab_out-idocnum = ls_remontada_idoc-idoc.
    MODIFY tab_out INDEX ls_remontada_index-index.
  ENDLOOP.
...
FORM event_double_click.
  ...
  PERFORM get_selected_line CHANGING gs_out l_selected_node.
  CHECK NOT gs_out IS INITIAL.
  ...
  CASE l_item_name.
    ...
    WHEN 'IDOCNUM'.
      CHECK NOT gs_out-idocnum IS INITIAL.
      CALL FUNCTION 'BAPI_IDOCAPPL_DISPLAY'
        EXPORTING
          idocnumber            = gs_out-idocnum
        EXCEPTIONS
          system_failure        = 01
          communication_failure = 02. "#EC *
    WHEN 'IDOCNUM_LIB'.
      CHECK NOT gs_out-idocnum_lib IS INITIAL.
      CALL FUNCTION 'BAPI_IDOCAPPL_DISPLAY'
        EXPORTING
          idocnumber            = gs_out-idocnum_lib
        EXCEPTIONS
          system_failure        = 01
          communication_failure = 02. "#EC *
  ENDCASE.
...
FORM altera_lock USING f_lock.
  ...
  DATA lt_lenum_aut  TYPE TABLE OF lenum.
  DATA lt_lenum_pick TYPE TABLE OF lenum.
  ...
  DATA: lt_pal_pick TYPE TABLE OF zwm026  WITH HEADER LINE.
...
**********************************************************************
  ...
  IF z_wm_cl_management=>is_reabastecimento_running( i_lgnum = lgnum i_refnr = gs_out-refnr ) EQ abap_true.
    " Reabastecimento em curso
    MESSAGE i360(zwmmsg001) WITH gs_out-refnr.
    EXIT.
  ENDIF.

  IF gs_out-zlock IS INITIAL.
*   Grupo & não liberado, volter a correr botão de Criação de Tarefas
    MESSAGE i361(zwmmsg001) WITH gs_out-refnr.
    EXIT.
  ENDIF.
...
*  IF g_lock EQ '3' OR g_lock EQ '4'.
*
*    " Validar Carga na Gravítica do Automático
*    IF st_destino EQ 'PUA' AND z_wm_cl_management=>is_allow_pua( EXPORTING i_lgnum = lgnum i_refnr = zwm028-refnr i_lock = g_lock IMPORTING et_messages = lt_messages ) EQ abap_false.
*      PERFORM show_message USING lt_messages.
*      RETURN.
*    ENDIF.
*  ENDIF.

  IF bin_destino1 EQ gc_dummy_adiamt_lgpla.
    CLEAR: bin_destino1.
...
** Validar Paletes e carateristicas da carga
**********************************************************************
  ...
  CLEAR: pal_picking, pal_aut, pal_manual, pal_esp.

  LOOP AT gt_out INTO ls_out WHERE refnr = g_grupo AND
                                   NOT tanum IS INITIAL.

    IF ls_out-vltyp EQ 'AUT' OR ls_out-vltyp EQ 'NPE'.
      pal_aut = pal_aut + 1.
      APPEND LINES OF z_wm_cl_management=>get_lenum( i_lgnum = lgnum is_data = ls_out ) TO lt_lenum_aut.
    ELSEIF z_wm_cl_management=>is_picking( i_lgnum = lgnum is_data = ls_out ).
      APPEND LINES OF z_wm_cl_management=>get_lenum( i_lgnum = lgnum is_data = ls_out ) TO lt_lenum_pick.
    ELSE.
      pal_manual = pal_manual + 1.
    ENDIF.
  ENDLOOP.

  SELECT *
   FROM zwm026 INTO TABLE lt_pal_pick
    WHERE armazem = lgnum
    AND   grupo   = g_grupo.

  SORT lt_pal_pick BY n_pal_picking.
  DELETE ADJACENT DUPLICATES FROM lt_pal_pick COMPARING n_pal_picking.

  DESCRIBE TABLE lt_pal_pick LINES pal_picking.

  SORT lt_lenum_aut.
  DELETE ADJACENT DUPLICATES FROM lt_lenum_aut.

  SORT lt_lenum_pick.
  DELETE ADJACENT DUPLICATES FROM lt_lenum_pick.

  LOOP AT lt_lenum_aut INTO lv_lenum.
    READ TABLE lt_lenum_pick
          TRANSPORTING NO FIELDS
          WITH KEY table_line = lv_lenum
          BINARY SEARCH.
    CHECK sy-subrc EQ 0.
    pal_picking = pal_picking - 1.
  ENDLOOP.

** Contagem de Paletes Pal. Especial
  CALL FUNCTION 'ZWM_COUNT_PAL_ESPECIAL'
    EXPORTING
      i_lgnum   = lgnum
      i_refnr   = g_grupo
    TABLES
  ...
  LOOP AT lt_pal_esp.
    pal_esp = pal_esp + lt_pal_esp-pal_esp.
  ENDLOOP.

** Sem Pulmão atribuido
**********************************************************************
  IF zwm028-st_pul IS INITIAL.
    ...
    IF pal_aut > 0 AND pal_aut > pal_manual AND z_wm_cl_management=>is_allow_pua( i_lgnum = zwm028-lgnum i_refnr = zwm028-refnr i_lock = g_lock ) EQ abap_true..
      texto0400 = 'Característica de AUT!'.
      CLEAR caract_carga.
    ENDIF.


    CALL SCREEN '0400' STARTING AT 10 1 ENDING AT 63 15.
    CHECK g_ok_code = 'CONFIRMAR'.

** Com Pulmao atribuido
**********************************************************************
  ELSE.
    IF g_lock = 5.
      CHECK zwm028-st_dck IS INITIAL.
...
** Atualizar tabela dos grupos
  DELETE gt_zwm028 WHERE refnr = gs_out-refnr.

  SELECT *
    FROM zwm028 APPENDING TABLE gt_zwm028
    WHERE lgnum = lgnum
    AND   refnr = gs_out-refnr.

  SORT gt_zwm028 BY lgnum refnr remessa.

** Atualizar Nó do ALV
  PERFORM atribui_icon_lock_grupo
                   USING gs_out-refnr f_lock
                CHANGING gs_out-icon_lock gs_out-icon_created.

  PERFORM ler_texto_lock_grupo
                  USING f_lock
               CHANGING gs_out-desc_lock.

  IF NOT st_destino IS INITIAL AND
     NOT bin_destino1 IS INITIAL.

    CONCATENATE st_destino bin_destino1 INTO gs_out-destino SEPARATED BY space.
  ...
  PERFORM change_nodes_idoc_number USING l_selected_node.
...
*      -->F_OUT      text
*---------------------------------------------------------------------*
FORM atribui_icon_lock_grupo USING f_refnr TYPE lvs_refnr f_lock f_out f_created.

  DATA: lv_created   TYPE kostk,
        lv_completed TYPE flag,
        ls_zwm028    TYPE zwm028.

  DATA: lt_ltap TYPE TABLE OF ltap WITH HEADER LINE.
  DATA: lt_ltak TYPE TABLE OF ltak WITH HEADER LINE.

  READ TABLE gt_zwm028 INTO ls_zwm028 WITH KEY refnr = f_refnr.
*  SELECT SINGLE *
*    FROM zwm028 INTO ls_zwm028
*    WHERE lgnum = lgnum
*    AND   refnr = f_refnr.
  ...
  IF sy-subrc <> 0.
    MOVE icon_led_red TO f_created.
  ELSE.
    IF z_wm_cl_management=>is_reabastecimento_running( i_lgnum = lgnum i_refnr = f_refnr ) EQ abap_true.
      MOVE icon_pm_insert TO f_out.
      MOVE icon_space TO f_created.
      EXIT.
    ENDIF.

    lv_completed = z_wm_cl_management=>is_group_completed( EXPORTING i_lgnum = lgnum i_refnr = f_refnr IMPORTING e_created = lv_created ).

    CASE lv_created.
      WHEN 'C'.
        MOVE icon_led_green  TO f_created.
      WHEN 'B'.
        MOVE icon_led_yellow TO f_created.
      WHEN OTHERS.
        MOVE icon_led_red    TO f_created.
    ENDCASE.
  ENDIF.
...
*  IF sy-subrc <> 0.
*    MOVE icon_led_red TO f_created.
**  ELSEIF ls_zwm028-kzdru IS INITIAL.
**    MOVE icon_led_yellow TO f_created.
*  ELSE.
*    MOVE icon_led_green TO f_created.
*  ENDIF.
...
*  ENDIF.
*
*  " Existiu um estorno - Validar Ots do Grupo
*  IF ls_zwm028-kzdru IS NOT INITIAL.
*    LOOP AT lt_ltap WHERE vorga = 'SL' OR vorga = 'ST'.
*      lv_completed = z_wm_cl_management=>is_group_completed( EXPORTING i_lgnum = lgnum i_refnr = f_refnr IMPORTING e_created = lv_created ).
*      EXIT.
*    ENDLOOP.
*
...
*  ENDIF.
  ...
  IF ( f_lock EQ 0 OR f_lock EQ 1 ) AND lv_completed EQ abap_false.
    MOVE icon_workflow_external_event TO f_out.
    EXIT.
  ENDIF.
...
FORM get_costumizing .
  ...
  PERFORM get_parameter
          USING lgnum
                ...
                'FILA_SAIDA_AUT'
                valor.
  MOVE valor TO aut_queue.

  aut_queue_tri = 'QUEUEST_G'.
...
* add node
  ...
  CLEAR: ls_out-total_paletes, ls_out-paletes_pulmao,
         ...
         ls_out-paletes_crd, ls_out-paletes_blk.
...
FORM processa_to_partida .
  ...
  IF NOT gs_out-to_partida IS INITIAL.
    ...
    PERFORM check_bloqueios_grupo CHANGING lv_bloq.

    CHECK lv_bloq IS INITIAL.
    ...
    ELSE.
      ...
      IF z_wm_cl_management=>is_remontada( is_data = ltap ) EQ abap_true.
        CLEAR zwm020.
    ...
    PERFORM check_desbloqueio_grupo.
...
MODULE status_0400 OUTPUT.
  ...
  LOOP AT SCREEN.
    IF screen-name = 'ST_DESTINO'.
      IF st_destino <> 'PLM' AND st_destino <> 'AUT' AND st_destino <> 'BPK'.
    ...
    IF screen-name = 'LB_DESTINO2'.
      IF zwm028-lgnum EQ '100'.
        screen-invisible = 1.
        MODIFY SCREEN.
        CONTINUE.
      ENDIF.
...
MODULE check_st_destino1 INPUT.
  ...
  DATA: lt_vbpa     LIKE vbpa OCCURS 0 WITH HEADER LINE,
        ...
        lt_zwm006   TYPE TABLE OF zwm006_aux WITH HEADER LINE.
...
************************************************************************
  ...
  IF ( st_destino = 'AUT' OR  st_destino = 'BPK' ) AND g_lock <> '2'.
**  Desbloqueio de & somente possivel em desbloqueio de & (&)
    MESSAGE ID 'ZWMMSG001' TYPE 'I' NUMBER '359' WITH 'AUT/BPK' 'Picking' '2'.
    CLEAR: ok_code_0400, st_destino.
    EXIT.
  ENDIF.

  IF st_destino = 'PUA' AND g_lock <> '4' AND g_lock <> '3'.
**  Desbloqueio de & somente possivel em desbloqueio de & (&)
    MESSAGE ID 'ZWMMSG001' TYPE 'I' NUMBER '359' WITH 'PUA' 'Picking & Completas' '3 ou 4'.
    CLEAR: ok_code_0400, st_destino.
    EXIT.
  ENDIF.

  IF st_destino = 'PUA' AND z_wm_cl_management=>is_allow_pua( EXPORTING i_lgnum = zwm028-lgnum i_refnr = zwm028-refnr  i_lock = g_lock IMPORTING et_messages = lt_messages ) <> abap_true.
    PERFORM show_message USING lt_messages.
    CLEAR: ok_code_0400, st_destino.
    EXIT.
  ENDIF.

  IF st_destino = 'PUL' AND z_wm_cl_management=>is_allow_pul( EXPORTING i_lgnum = zwm028-lgnum i_refnr = zwm028-refnr  i_lock = g_lock IMPORTING et_messages = lt_messages ) <> abap_true.
    PERFORM show_message USING lt_messages.
    CLEAR: ok_code_0400, st_destino.
    EXIT.
  ENDIF.
...
** Cargas apenas com paletes completas não valida se é servisan
  ...
  IF st_destino <> 'DCK'.
    ...
    IF g_lock EQ '2' AND
    ( st_destino EQ 'AUT' OR st_destino EQ 'PUA' OR st_destino EQ 'PEX' OR st_destino EQ 'BPK' ).
      bin_destino1 = gc_dummy_adiamt_lgpla.
      EXIT.
    ENDIF.
...
MODULE user_command_0400 INPUT.
  ...
  DATA lv_free_only_wcs TYPE flag.
  DATA lv_wcs_free TYPE c.
...
** Validar Paletização Especial
  ." OR g_lock EQ '3' OR g_lock EQ '4'.

*  IF pal_esp IS NOT INITIAL.
*    PERFORM check_pal_esp USING gs_out-refnr lv_subrc.
*
*    CHECK lv_subrc = 0.
*  ENDIF.
...
*     ET_MESSAGES   =
  ...
  CASE st_destino.
    ...
    WHEN 'DCK'.
      ...
      " Paletes de Picking no BPK
      IF zwm028-st_pul = 'BPK'.
        UPDATE zwm028 SET st_pul     = ''
                          pulmao1    = ''
                     WHERE lgnum = lgnum
                       AND refnr = gs_out-refnr.
...
**  Verificar se existem mais remessas com o mesmo recebedor de merc.
      ...
      ENDIF.
    WHEN 'PUA' OR 'PEX' OR 'AUT' OR 'BPK'.

      IF st_destino EQ 'PUA' OR st_destino EQ 'AUT'.
        CLEAR: lv_free_only_wcs.
        lv_wcs_free = 'A'.
      ENDIF.
  ...
  IF g_lock EQ '3' OR g_lock EQ '4'.
    ...
    CALL FUNCTION 'ZWM_IDOC_FREE_WORK_WCS'
      EXPORTING
        ...
        i_refnr        = gs_out-refnr
        i_step         = '1'
        i_unlock_first = abap_true.

** Validar Reserva do WCS
    SELECT SINGLE *
    ...
    IF sy-subrc = 0.
      ...
      DO lv_times TIMES.
        ...
        IF z_wm_cl_management=>is_allow_free_work( EXPORTING i_lgnum = lgnum i_refnr = gs_out-refnr
                                                   IMPORTING et_messages = lt_messages ) <> abap_true.
...
*                  lgtyp = 'PLM' OR
  ...
  IF g_lock EQ '2'.
    DELETE posicoes WHERE lgtyp = 'PUA'.
    DELETE posicoes WHERE lgtyp = 'PEX'.
    CLEAR posicoes.
    posicoes-lgnum = zwm028-lgnum.
    posicoes-lgtyp = 'AUT'.
    APPEND posicoes.
  ...
  IF z_wm_cl_management=>is_allow_pua( i_lgnum = zwm028-lgnum i_refnr = zwm028-refnr  i_lock = g_lock ) <> abap_true.
    DELETE posicoes WHERE lgtyp = 'PUA'.
    DELETE posicoes WHERE lgtyp = 'PEX'.
    DELETE posicoes WHERE lgtyp = 'AUT'.
  ENDIF.
...
*----------------------------------------------------------------------*
FORM get_desbloqueio_carga .
  DATA lv_valor TYPE zwm001-valor.
  ...
  DO 1 TIMES.
    ...
    ELSEIF st_destino = 'PLM'.
      g_desbloq = 'G'.
    ELSEIF st_destino = 'PUA' OR st_destino = 'AUT'.
      g_desbloq = 'R'.
    ELSEIF st_destino = 'PEX'.
      IF zwm028-total_paletes > 17.
        g_desbloq = 'R'.
      ELSE.
        g_desbloq = 'G'.
      ENDIF.
    ELSEIF st_destino = 'BPK'.
      g_desbloq = 'G'.
    ENDIF.
...
** Desbloqueio de loja Online com remessas RP12
**********************************************************************
  LOOP AT tab_out WHERE refnr = zwm028-refnr AND vkorg <> 'RP12'.
    EXIT.
  ENDLOOP.
  IF sy-subrc <> 0.
    LOOP AT tab_out WHERE refnr = zwm028-refnr AND vkorg = 'RP12'.
...
FORM load_car .
  ...
  IF lt_zwm028-st_pul = 'PUA'.
    MESSAGE ID 'ZWMMSG001' TYPE 'E' NUMBER '378'.
    EXIT.
  ENDIF.
...
**********************************************************************
  ...
  CLEAR range_refnr.
  MOVE:
         gs_out-refnr TO range_refnr-low,
        'I'           TO range_refnr-sign,
        'EQ'          TO range_refnr-option.
  APPEND range_refnr.
  ...
  CALL FUNCTION 'POPUP_TO_CONFIRM_STEP'
    EXPORTING
      defaultoption  = 'N'
      textline1      = 'Deseja avançar com a criação de OTs?'
      titel          = 'Criação de OTs.'
      cancel_display = ' '
    IMPORTING
      answer         = lv_rsp.
...
** Validar Paletização Especial do Automático
...
** Validar Waves Picking
  CALL FUNCTION 'ZWM_WAVES_PICKING'
    EXPORTING
      i_lgnum     = lgnum
      i_refnr     = gs_out-refnr
    IMPORTING
...
ENDFORM.
*&---------------------------------------------------------------------*
*&      Form  GET_SELECTED_LINE
*&---------------------------------------------------------------------*
*       text
*----------------------------------------------------------------------*
*      <--P_GS_OUT  text
*----------------------------------------------------------------------*
FORM get_selected_line CHANGING cs_out           TYPE t_out
                                cv_selected_node TYPE lvc_nkey.
  DATA: l_selected_node TYPE lvc_nkey,
        l_item_name     TYPE lvc_fname.

  DATA: l_index         TYPE lvc_index.

  DATA: lref_result TYPE REF TO data.

  FIELD-SYMBOLS: <ls_data> TYPE any.

  CLEAR: cs_out, cv_selected_node.

  CREATE DATA lref_result TYPE  t_out.
  ASSIGN lref_result->* TO <ls_data>.
  ...
  CHECK NOT l_selected_node IS INITIAL.
  ...
  cv_selected_node = l_selected_node.
ENDFORM.
*&---------------------------------------------------------------------*
*&      Form  SHOW_MESSAGE
*&---------------------------------------------------------------------*
...
FORM resend_idoc .
  ...
  PERFORM get_selected_line CHANGING gs_out lv_selected_node.
  CHECK NOT gs_out IS INITIAL.
  CHECK NOT gs_out-tanum IS INITIAL.
...
FORM free_work.
  ...
  DATA: lv_selected_node TYPE lvc_nkey,
        ...
        lv_text1         TYPE char100,
        lv_text2         TYPE char100,
        lv_return        TYPE char1,
        lv_ot_man        TYPE flag,
        lv_return_a      TYPE char1,
        ls_vttp          TYPE vttp,
        ls_ltap          TYPE ltap,
        ls_zwm028        TYPE zwm028.

  DATA: lt_zwm028        TYPE TABLE OF zwm028     WITH HEADER LINE.
  DATA: lt_zwm078        TYPE TABLE OF zwm078     WITH HEADER LINE.
  DATA: lt_zwm006        TYPE TABLE OF zwm006_aux WITH HEADER LINE.
...
**********************************************************************
  ...
  lv_refnr = gs_out-refnr.

  IF z_wm_cl_management=>is_group_completed( i_lgnum = lgnum i_refnr = lv_refnr ) EQ abap_false.

    PERFORM confirm_step USING 'Grupo sem todas OTs Criadas' 'Deseja avançar?' '' lv_return_a.
    CHECK lv_return_a EQ 'J'.
  ...
  LOOP AT lt_zwm078 INTO ls_zwm078 WHERE reserva IS INITIAL AND estorno IS INITIAL.

    SELECT SINGLE *
      FROM ltap INTO ls_ltap
      WHERE lgnum = ls_zwm078-lgnum
      AND   tanum = ls_zwm078-tanum
      AND   tapos = ls_zwm078-tapos.

    " Eliminar OTs estornadas
    IF sy-subrc <> 0 OR ( ls_ltap-vorga = 'ST' OR ls_ltap-vorga = 'SL' ).

      UPDATE zwm078 SET estorno = 'X'
      WHERE lgnum = ls_zwm078-lgnum
      AND   tanum = ls_zwm078-tanum
      AND   tapos = ls_zwm078-tapos.
      IF sy-subrc = 0.
...
** Validar Liberação
**********************************************************************
  IF z_wm_cl_management=>is_allow_free_work( EXPORTING i_lgnum = lgnum i_refnr = lv_refnr IMPORTING et_messages = lt_messages ) <> abap_true.
...
** Validar Carga na gravítica com mais de 36 paletes
  SELECT * INTO TABLE lt_zwm078
       FROM zwm078
           WHERE lgnum = lgnum
             AND refnr = lv_refnr.

  SORT lt_zwm078 BY tanum.
  DELETE ADJACENT DUPLICATES FROM lt_zwm078 COMPARING tanum.

  SORT lt_zwm078 BY ordem tanum.

  " OTs do Armazém Manual
  LOOP AT lt_zwm078.
    CHECK sy-tabix > 36.

    IF lt_zwm078-exp <> 'AUT'.
      lv_ot_man = 'X'.
      EXIT.
    ENDIF.
  ENDLOOP.
  ...
  IF lv_ot_man = 'X'.
    ...
    " Validar se Camião já tem porta atribuida
    IF lt_zwm028-st_pul = 'PUA' AND lt_zwm028-total_paletes > '36'.

      SELECT SINGLE *
        FROM vttp INTO ls_vttp
        WHERE vbeln = lt_zwm028-remessa.

      IF sy-subrc = 0.
        SELECT *
          FROM zwm006_aux INTO TABLE lt_zwm006
          WHERE armazem      = lgnum
          AND   n_transporte = ls_vttp-tknum.

        DELETE lt_zwm006 WHERE finalizada IS NOT INITIAL.

        CLEAR lt_zwm006.
        READ TABLE lt_zwm006 INDEX 1.

        IF lt_zwm006-porta IS INITIAL.

          lv_text1 = 'Carga na gravitica > 36 paletes. Tem paletes do armazém Manual.'.
          lv_text2 = 'Transporte & ainda não tem porta atribuida. Deseja avançar ?'.

          REPLACE '&' INTO lv_text2 WITH ls_vttp-tknum.

          PERFORM confirm_step USING lv_text1 lv_text2  '' lv_return.
          CHECK lv_return EQ 'J'.
  ...
  UPDATE zwm028 SET zlock = '4'
                WHERE lgnum = lgnum
                  AND refnr = lv_refnr
                  AND ordem = '01'.
  COMMIT WORK AND WAIT.

** Trabalho Liberado com sucesso
  MESSAGE s362(zwmmsg001).
...
*      -->P_L_SUBRC  text
*----------------------------------------------------------------------*
FORM check_pal_esp USING f_grupo f_subrc.

  DATA: lv_benum    TYPE lvs_benum.
  DATA: lv_return   TYPE c.
  DATA: lv_f_error  TYPE flag.
  DATA: lv_lgort    TYPE lgort_d.
  DATA: lv_werks    TYPE werks_d.
  DATA: lv_bwlvs    TYPE bwlvs.
  DATA: lv_prio     TYPE zwme_prioridades.
...
*  IF st_destino = 'AUT'.
*    " Grupo & com Paletes de Paletização Especial na estação do automático
*    MESSAGE ID 'ZWMMSG001' TYPE 'I' NUMBER '376' WITH f_grupo.
*  ENDIF.

** NT Por Criar - Validar Criação de NT
**********************************************************************
  SELECT SINGLE *
  ...
  IF sy-subrc <> 0.
    ...
    CHECK ls_zwm028-x_epe IS INITIAL AND g_lock EQ '2'.

    PERFORM confirm_step USING 'Grupo com Paletização Especial'
                               'Deseja Paletizar na Estação do Automático?'
                               'Paletização Especial'
                                lv_return.
    IF lv_return EQ 'J'.

** Estornar OTs de expedição dos Materiais que são para Paletizar automaticamente
      CALL FUNCTION 'ZWM_EST_PAL_ESPECIAL'
        EXPORTING
          i_lgnum     = lgnum
          i_refnr     = f_grupo
        IMPORTING
...
** Validar Número de Paletes de Pal. Especial
      CALL FUNCTION 'ZWM_COUNT_PAL_ESPECIAL'
        EXPORTING
          i_lgnum     = lgnum
          i_refnr     = f_grupo
...
** Criar NT e OT para Buffer de Paletização Especial
      PERFORM get_parameter USING lgnum 'GERAL' 'PLANT' lv_werks.
      PERFORM get_parameter USING lgnum 'GERAL' 'LGORT' lv_lgort.

      PERFORM get_parameter USING lgnum 'PALETIZACAO_ESPECIAL' 'MOV_WM_NT' lv_bwlvs.

      SELECT SINGLE prioridade
        FROM zwm028 INTO lv_prio
        WHERE lgnum   = lgnum   AND
              refnr   = f_grupo AND
              remessa = space.
      ...
      LOOP AT lt_pal_esp.
        ...
        lt_ltba-bwlvs = lv_bwlvs.
        lt_ltba-betyp = 'S'.
        lt_ltba-benum = lt_pal_esp-refnr.
        lt_ltba-lznum = lt_pal_esp-lznum.
        lt_ltba-lgnum = lgnum.
        lt_ltba-werks = lv_werks.
        lt_ltba-lgort = lv_lgort.
        lt_ltba-matnr = lt_pal_esp-matnr.
        lt_ltba-menga = lt_pal_esp-lfimg.
        lt_ltba-altme = lt_pal_esp-vrkme.
        lt_ltba-tbpri = lv_prio.
...
** NT Criada - Validar se Paletes de Pal. Especial estão todas no automático
**********************************************************************
  ELSE.

    CHECK g_lock EQ '3' OR g_lock EQ '4'.

    IF st_destino = 'PUA' OR ( st_destino <> 'PUA' AND ls_zwm028-st_pul = 'AUT' ).
...
ENDFORM.
*&---------------------------------------------------------------------*
*&      Form  CHECK_BLOQUEIOS_GRUPO
*&---------------------------------------------------------------------*
*       text
*----------------------------------------------------------------------*
*      <--P_LV_BLOQ  text
*----------------------------------------------------------------------*
FORM check_bloqueios_grupo CHANGING pv_bloq.
...
** Criar bloqueio
**********************************************************************
  MOVE 'TAREFAS_ZWMREP0024B' TO lv_key.
  ...
  IF pv_bloq IS NOT INITIAL.
    MESSAGE i366 WITH sy-msgv1 'Criação de Tarefas' DISPLAY LIKE 'W'.
  ENDIF.

ENDFORM.
*&---------------------------------------------------------------------*
*&      Form  check_desbloqueio_grupo
*&---------------------------------------------------------------------*
*       text
*----------------------------------------------------------------------*
*  -->  p1        text
...
FORM mapa_planeamento.
  ...
  DATA: lt_out TYPE t_out OCCURS 0.
  DATA: lt_t311 TYPE TABLE OF t311 WITH HEADER LINE.

  RANGES: range_refnr FOR t311-refnr.
  RANGES: range_datum FOR t311-datum.

** Obter grupos sem ots
**********************************************************************
  LOOP AT gt_out INTO gs_out WHERE icon_created = icon_led_red.
  ...
  SORT lt_out BY refnr.

  DELETE ADJACENT DUPLICATES FROM lt_out COMPARING refnr.
...
** Grupos
  SELECT *
    ...
    WHERE lgnum = lgnum
    AND   refnr = lt_out-refnr.

  SORT lt_t311 BY refnr.
  ...
  LOOP AT lt_t311.
    CLEAR range_refnr.
    MOVE:
           lt_t311-refnr TO range_refnr-low,
          'I'            TO range_refnr-sign,
          'EQ'           TO range_refnr-option.
    APPEND range_refnr.
  ENDLOOP.
...
** Data
  SORT lt_t311 BY datum.

  READ TABLE lt_t311 INDEX 1.
  IF sy-subrc = 0.
    CLEAR range_datum.

    MOVE:
           lt_t311-datum TO range_datum-low,
          'I'            TO range_datum-sign,
          'GE'           TO range_datum-option.
    APPEND range_datum.
  ENDIF.
*
*DATA: Lt_bdcdata TYPE bdcdata OCCURS 0 WITH HEADER LINE.
*
*Lt_bdcdata-FNAM = 'S_REFNR-LOW'
*
* CALL TRANSACTION 'ZWM166' AND SKIP FIRST SCREEN.

  SUBMIT zwmrep0090 WITH s_refnr IN range_refnr
                    WITH s_datum IN range_datum
                    AND RETURN.

ENDFORM.
*&---------------------------------------------------------------------*
*&      Form  CHANGE_NODES_IDOC_NUMBER
*&---------------------------------------------------------------------*
*       text
*----------------------------------------------------------------------*
*      -->P_L_SELECTED_NODE  text
*----------------------------------------------------------------------*
FORM change_nodes_idoc_number  USING pv_selected_node.

  DATA: ls_out TYPE t_out.

  PERFORM get_selected_line CHANGING ls_out pv_selected_node.

  CHECK NOT ls_out IS INITIAL.

*  CALL METHOD tree1->change_node
*    EXPORTING
*      i_node_key     = pv_selected_node
*      i_outtab_line  = ls_out
*    EXCEPTIONS
*      node_not_found = 1
*      OTHERS         = 2.
