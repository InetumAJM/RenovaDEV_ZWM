REPORT zwmrep0008 MESSAGE-ID zwmmsg001.
...
AT SELECTION-SCREEN ON p2_passw.
  IF p_passwd NE p2_passw.
    MESSAGE ID 'ZWMMSG001' TYPE 'E' NUMBER '067'.
  ENDIF.
...
AT SELECTION-SCREEN ON p_lgpla.
  ...
  IF NOT p_lgpla-high IS INITIAL.
    IF p_lgpla-low+8(2) <> p_lgpla-high+8(2).
      MESSAGE ID 'ZWMMSG001' TYPE 'E' NUMBER '72'.
    ENDIF.
  ENDIF.
...
START-OF-SELECTION.
  ...
  IF p_bin = 'X'.
    IF p_lgpla IS INITIAL OR p_lgtyp IS INITIAL.
      MESSAGE ID 'ZWMMSG001' TYPE 'E' NUMBER '999'
      WITH 'É obrigatório preencher todos os campos.'.
    ENDIF.
  ELSE.
...
*  LVS_ITCPO-TDDEST = 'Zz01'.
*
*  LVS_ITCPO-TDIMMED    = 'X'.
*  LVS_ITCPO-TDLIFETIME = '0'.
*  LVS_ITCPO-TDDELETE   = 'X'.
*  LVS_ITCPO-TDPROGRAM  = 'yxwmptrep0014'.
*
...
** colocados no ecrã de selecção e não por nivel
    ...
    DO aux_linhas TIMES.
      ...
      IF indice <= linhas.
        READ TABLE tbins INDEX indice.
        MOVE tbins-lgpla TO bin2.
        MOVE tbins-lgpla TO bin2_aux.
      ENDIF.
