FUNCTION z_wmpt_ws_asynchronous_lite.
*"----------------------------------------------------------------------
*"*"Interface local:
*"  IMPORTING
...
*"----------------------------------------------------------------------
  ...
  FIELD-SYMBOLS <fs_zwmt001>  LIKE LINE OF gt_zwmt001[].
*"----------------------------------------------------------------------
  ...
  READ TABLE gt_zwmt001[] ASSIGNING <fs_zwmt001>
    WITH TABLE KEY processo   = c_process_edi_wm_auto
                   parametro  = c_param_edi_wm_sndpor
                   item       = 0.
  IF sy-subrc EQ 0.
    lv_ediport  = <fs_zwmt001>-valor.
  ELSE.
    lv_message = 'Não foi possível obter o sistema lógico EDI.'.
  ...
  READ TABLE gt_zwmt001[] ASSIGNING <fs_zwmt001>
    WITH TABLE KEY processo   = c_process_edi_wm_auto
                   ...
                   item       = 0.
  IF sy-subrc EQ 0.
    lv_edienvi  = <fs_zwmt001>-valor.
  ELSE.
    lv_message = 'Não foi possível obter o ambiente SAP EDI.'.
    lo_ballog->log_add_message( ).
    lo_ballog->log_save( ).
