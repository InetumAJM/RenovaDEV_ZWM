...
*  Data:  28.04.2005                                                   *
*  Descrição: Lista de TO's por Queues.                                *
*
* Modificações
*  Autor:
*  Data:
...
REPORT zwmrep0052 MESSAGE-ID zwmmsg001 .
...
FORM build_fieldcatalog.
  ...
  CLEAR aux_cat.
  aux_cat-col_pos       =  pos.
  aux_cat-fieldname     = 'ENAME'.
  aux_cat-key           = 'X'.
  aux_cat-no_zero       = 'X'.
  aux_cat-do_sum        = 'X'.
  aux_cat-coltext       = 'User 1ª conf.TO'.
  aux_cat-just          = 'L'.
  APPEND aux_cat.
  ADD 1 TO pos.
  ...
  aux_cat-no_zero       = 'X'.
  aux_cat-do_sum        = 'X'.
  aux_cat-coltext       = 'Hora 1ª conf.TO'.
  aux_cat-just          = 'L'.
  APPEND aux_cat.
  ADD 1 TO pos.
...
* Alteração Ferreira 2007.04.11
  CLEAR aux_cat.
  aux_cat-col_pos       =  pos.
  aux_cat-fieldname     = 'QNAME'.
  aux_cat-key           = 'X'.
  aux_cat-no_zero       = 'X'.
  aux_cat-do_sum        = 'X'.
  aux_cat-coltext       = 'User 2ª conf.TO'.
  aux_cat-just          = 'L'.
  APPEND aux_cat.
  ADD 1 TO pos.
  ...
  CLEAR aux_cat.
  aux_cat-col_pos       =  pos.
  aux_cat-fieldname     = 'QZEIT'.
  aux_cat-key           = 'X'.
  aux_cat-no_zero       = 'X'.
  aux_cat-do_sum        = 'X'.
  aux_cat-coltext       = 'Hora 2ª conf.TO'.
  aux_cat-just          = 'L'.
  APPEND aux_cat.
  ADD 1 TO pos.
* Fim alteração Ferreira
...
FORM build_hierarchy_header CHANGING
  ...
  p_hierarchy_header-heading = 'Data Criação/Equipamento/Fila'.
  p_hierarchy_header-tooltip = 'Data Criação/Equipamento/Fila'.
...
** Não lista se não existirem dados
  ...
  SORT itab BY lgnum data equipamento fila status_to tanum tapos.
...
FORM select_data.
  ...
  SORT itab_dados BY bdatu queue.
...
** Nº de TO só criadas
      READ TABLE itab_to WITH KEY bdatu     = ps_out-bdatu
                                  fila      = ps_out-fila
                                  status_to = 'C'.
      IF sy-subrc EQ 0.
        MOVE itab_to-num_to TO ls_out-total_to.
      ENDIF.
...
** Nº Total de TO
  LOOP AT itab_data WHERE bdatu EQ ps_out-bdatu.
    LOOP AT itab_to WHERE bdatu = itab_data-bdatu
                      AND fila  = itab_data-queue.
