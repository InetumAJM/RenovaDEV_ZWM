FUNCTION zwm_delivery_batch_split.
*"----------------------------------------------------------------------
*"*"Interface local:
*"  IMPORTING
...
*      IF lt_lips-charg <> lt_vepo-charg.
    ...
    " Validar se lote já existe
    READ TABLE lt_lips_aux INTO ls_lips WITH KEY uecha = lt_lips-posnr
                                                 charg = lt_vepo-charg.
    IF sy-subrc = 0.
      CLEAR lt_item_data.
      lt_item_data-deliv_numb      = ls_lips-vbeln.
      lt_item_data-deliv_item      = ls_lips-posnr.
      lt_item_data-material        = ls_lips-matnr.
      lt_item_data-batch           = ls_lips-charg.
